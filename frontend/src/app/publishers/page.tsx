...
export default function PublisherPage(){
  ...
  useEffect(() => {
    async function fetchPublishers(){
        try{
          const publishersData = await axios.get<Publisher[]>('http://localhost:8000/api/publishers/')
    ...
    <main style={{ padding: 20 }}>
      ...
      {formVisible && (
        <div style={{ marginBottom: 30, border: '1px solid #ccc', padding: 15, maxWidth: 400 }}>
          ...
          <textarea
            ...
            rows={4}
            style={{ marginTop: 10, width: '100%', padding: 8, boxSizing: 'border-box' }}/>
      ...
      <ul>
        {publishers.map((publisher) => (
          <li key={publisher.id}>
            {publisher.id} - {publisher.name} - {publisher.address ?? 'No publisher address'}
          </li>
        ))}
      </ul>
    </main>
